export default function Repo({name, description, language, tags, selectedTag, html_url, homepage}) {
  ...
  return <div className={`repo ${activeState}`}>
    <div className="repo-content">
      ...
      <img src={`${html_url}/blob/master/thumb.png?raw=true`} />
      <p><span className="language">{language}</span> - {description.split('|')[0]}</p>
    </div>
    <div className="repo-buttons">
      
      {homepage && <a href={homepage} target="_blank"><button className="page-button">Acessar página</button></a>}
      <a href={html_url} target="_blank"><button className="repo-button">Acessar repositório</button></a>
    </div>
  </div>;
}